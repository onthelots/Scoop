...
protocol MyPostCategoryViewDelegate: AnyObject {
    func myPostCategoryLabelTapped(_ gesture: UITapGestureRecognizer)
}

class MyPostCategoryView: UIView {

    weak var delegate: MyPostCategoryViewDelegate?
    ...
    @objc private func categoryLabelTapped(_ gesture: UITapGestureRecognizer) {
        delegate?.myPostCategoryLabelTapped(gesture)
    }
}
