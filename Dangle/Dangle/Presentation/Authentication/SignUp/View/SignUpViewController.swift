    ...
    private func setupBindings() {
        ...
        viewModel.$isEmailValid
            ...
            .sink { [weak signUpView] isValid in
                signUpView?.emailValidationLabel.textColor = isValid ? .green : .red
            }
            .store(in: &subscription)
        ...
        viewModel.$isPasswordContainsLetter
            .receive(on: RunLoop.main)
            .sink { [weak signUpView] isValid in
                signUpView?.passwordContainsLetterLabel.textColor = isValid ? .green : .lightGray
            }
            .store(in: &subscription)
        ...
        //            .combineLatest(viewModel.$isPasswordContainsLetter) { $0 && $1 }
            .receive(on: RunLoop.main)
            .sink { [weak signUpView] isValid in
                signUpView?.passwordContainsNumberLabel.textColor = isValid ? .green : .lightGray
            }
            .store(in: &subscription)
        ...
        viewModel.$isPasswordLengthValid
            .receive(on: RunLoop.main)
            .sink { [weak signUpView] isValid in
                signUpView?.passwordLengthLabel.textColor = isValid ? .green : .lightGray
            }
            .store(in: &subscription)
        ...
        viewModel.$isSignUpButtonEnabled
            ...
            .sink { [weak signUpView] isEnabled in
                ...
                signUpView?.nextButton.tintColor = isEnabled ? .systemBlue : .gray
            }
            .store(in: &subscription)
